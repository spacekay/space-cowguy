...
public class BackTracking9 {
	...
	public static void main(String[] args) throws IOException {
		...
		DFS(0,result,visit,letter);
		bw.flush();
		bw.close();
	}
	public static void DFS(int level, String result, boolean[] visit, boolean[][] letter) throws IOException {
		if(level==m) {
			bw.write(result+"\n");
		} else {
			if (level>0)
			...
			for(int i=0;i<n;i++) {
				if(!visit[i]&&!letter[level][arr[i]]) {			
					visit[i]=true;
					// 이전에 true로 만든 letter 배열이 전달됨
					DFS(level+1, result+Integer.toString(arr[i]), visit, letter);	
